...
export const useFaceAnalysisStore = create((set, get) => ({
  analysisData: null,
  analysisId: null,
  ...
  startAnalysis: async (imageUrl) => {
    ...
    try {
      const data = await faceAnalysisService.startAnalysis(imageUrl);
      set({ 
  ...
  fetchAnalysis: async (analysisId) => {
    ...
    try {
      ...
      const data = await faceAnalysisService.getAnalysis(currentId);
      set({ 
        analysisData: data,
